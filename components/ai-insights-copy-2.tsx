...
import { 
  ...
  Share2, Printer, MessageSquare, Edit3, RefreshCw, Loader2, AlertTriangle, Siren // Tambahkan Siren
} from 'lucide-react'; //
import {
  Dialog,
  DialogContent,
...
// --- TAMBAHAN: Interface untuk Error Analysis Report ---
...
interface AiInsightsData { //
  ...
  errorAnalysisReport?: ErrorAnalysisDetail[]; // Tambahkan ini
}
            ...
            <Tabs value={currentTab} onValueChange={setCurrentTab} className="w-full"> {/* */}
              <ScrollArea className="w-full whitespace-nowrap rounded-md border-b dark:border-gray-700"> {/* */}
                <TabsList className="mb-0 flex-nowrap px-1 -mx-1"> {/* */}
                  ...
                  {/* --- MODIFIKASI: Tambah Tab atau gabung ke Tab Threats --- */}
              ...
              <div className="mt-4 md:mt-6">
                ...
                {/* --- MODIFIKASI: Tab Baru atau Gabungan --- */}
                <TabsContent value="errors_threats">
                  <div className="space-y-6">
                    {/* Bagian Error Analysis Report */}
                              ...
                              <AccordionItem value={`error-${index}`} key={`error-${index}`}>
                                <AccordionTrigger className="hover:no-underline">
                                  <div className="flex items-center justify-between w-full">
                                    <span className="font-semibold text-sm md:text-base">{errorDetail.errorType}</span>
                    ...
                    {/* Bagian Threat Analysis & IOCs (sama seperti sebelumnya, tapi masuk ke tab ini) */}
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center text-lg">
                                <Shield className="mr-2 text-red-500" /> General Threat Intelligence & IOCs
                                {data.threatLevel && (
                                    <Badge variant={ data.threatLevel.toLowerCase() === 'critical' || data.threatLevel.toLowerCase() === 'high' ? 'destructive' : data.threatLevel.toLowerCase() === 'medium' ? 'default' : 'outline' } className="ml-3 text-xs">
                        ...
                        <CardContent className="space-y-6">
                            <div>
                                <h3 className="font-semibold mb-2 text-base">Threat Analysis Summary:</h3>
                                <p className="prose dark:prose-invert max-w-none text-sm md:text-base">{data.threatAnalysis || "No general threat analysis provided by AI."}</p>
                            </div>
                            {data.findings && data.findings.length > 0 && (
                                <div>
                                    <h3 className="font-semibold mt-4 mb-2 text-base">Specific Security Findings:</h3>
                ...
                <TabsContent value="recommendations"> {/* */}
                  {/* ... (Konten Recommendations sama) ... */}
                  <Card><CardHeader><CardTitle className="flex items-center text-lg"><MessageSquare className="mr-2 text-green-500"/>Recommended Actions</CardTitle></CardHeader><CardContent className="prose dark:prose-invert max-w-none text-sm md:text-base">{data.recommendations&&Array.isArray(data.recommendations)&&data.recommendations.length>0?(<ul className="list-disc pl-5 space-y-2">{data.recommendations.map((rec,index)=>(<li key={index}><strong className="font-medium">{rec.title||`Recommendation ${index+1}`}</strong> (Priority: {rec.priority||"N/A"}):<p className="text-sm ml-4">{rec.description||"No detailed description."}</p></li>))}</ul>):(<p>{typeof data.recommendations==="string"?data.recommendations:"No specific recommendations provided by AI."}</p>)}</CardContent></Card>
                </TabsContent>

                <TabsContent value="timeline"> {/* */}
                  {/* ... (Konten Timeline sama) ... */}
                  <Card><CardHeader><CardTitle className="flex items-center text-lg"><Clock className="mr-2 text-fuchsia-500"/>Event Timeline</CardTitle></CardHeader><CardContent>{data.timeline&&data.timeline.length>0?(<div className="relative pl-6 space-y-6 border-l-2 border-gray-200 dark:border-gray-700">{data.timeline.map((event,index)=>(<div key={index}className="relative"><div className={`absolute -left-[calc(0.75rem+1px)] mt-1.5 flex h-6 w-6 items-center justify-center rounded-full ${event.severity==="error"?"bg-red-500":event.severity==="warning"?"bg-yellow-500":"bg-blue-500"} text-white text-xs font-semibold`}>{event.severity==="error"?<AlertCircle size={14}/>:event.severity==="warning"?<AlertTriangle size={14}/>:<Info size={14}/>}</div><div className="ml-4"><p className="font-medium text-sm">{event.event||"Unknown Event"}</p><p className="text-xs text-muted-foreground">{event.time&&!event.time.includes("Packet Sample")?new Date(event.time).toLocaleString():event.time||"N/A"}</p></div></div>))}</div>):(<p className="text-center text-gray-500 dark:text-gray-400 py-4">No timeline events provided by AI.</p>)}</CardContent></Card>
                </TabsContent>

                <TabsContent value="visuals" className="space-y-6"> {/* */}
                  {/* ... (Konten Visuals (PieChart, BarChart) sama) ... */}
                  {data.protocolDistribution&&data.protocolDistribution.length>0&&(<Card><CardHeader><CardTitle className="flex items-center text-lg"><PieChartIcon className="mr-2 text-purple-500"/>Protocol Distribution</CardTitle></CardHeader><CardContent style={{width:"100%",height:300}}><ResponsiveContainer><PieChart><Pie activeIndex={activePieIndex}activeShape={renderActiveShape}data={data.protocolDistribution}cx="50%"cy="50%"innerRadius={60}outerRadius={100}fill="#8884d8"dataKey="value"onMouseEnter={onPieEnter}>{data.protocolDistribution.map((entry,index)=>(<Cell key={`cell-${index}`}fill={entry.fill||COLORS[index%COLORS.length]}/>))}</Pie><RechartsTooltip/><Legend layout="vertical"align="right"verticalAlign="middle"iconSize={10}wrapperStyle={{fontSize:"12px"}}/></PieChart></ResponsiveContainer></CardContent></Card>)}
                  {data.statistics?.topTalkers&&data.statistics.topTalkers.length>0&&data.statistics.topTalkers[0].ip!=="No identifiable IP traffic"&&(<Card><CardHeader><CardTitle className="flex items-center text-lg"><BarChart2 className="mr-2 text-green-500"/>Top Talkers (by Packets)</CardTitle></CardHeader><CardContent style={{width:"100%",height:300}}><ResponsiveContainer><BarChart data={data.statistics.topTalkers}layout="vertical"margin={{top:5,right:30,left:20,bottom:5}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number"/><YAxis dataKey="ip"type="category"width={150}interval={0}tick={{fontSize:10}}/><RechartsTooltip/><Legend wrapperStyle={{fontSize:"12px"}}/><Bar dataKey="packets"name="Total Packets"fill="#82ca9d"/></BarChart></ResponsiveContainer></CardContent></Card>)}
                </TabsContent>

                {data.performanceMetrics && ( /* */
                  <TabsContent value="performance"> {/* */}
                    {/* ... (Konten Performance sama) ... */}
                    <Card><CardHeader><CardTitle className="flex items-center text-lg"><Activity className="mr-2 text-cyan-500"/>Performance</CardTitle></CardHeader><CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm md:text-base"><div><strong>Total Packets (in file):</strong> {data.performanceMetrics.totalPackets?.toLocaleString()||data.statistics?.totalPacketsInFile?.toLocaleString()||"N/A"}</div><div><strong>Total Bytes (in file):</strong> {data.performanceMetrics.totalBytes?.toLocaleString()||data.statistics?.totalBytesInFile?.toLocaleString()||"N/A"}</div><div><strong>Capture Duration:</strong> {data.performanceMetrics.captureDuration||"N/A"}</div><div><strong>Avg Packet Rate:</strong> {data.performanceMetrics.averagePacketRate||"N/A"}</div></CardContent></Card>
                  </TabsContent>
                )}
              </div>
            </Tabs>
